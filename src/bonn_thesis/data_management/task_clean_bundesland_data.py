...
def task_clean_bundesland_data(
    depends_on=SRC / "data" / "raw" / "05-staedte.xlsx",
    produces=BLD / "data" / "bundesland_data" / "bundesland_reference.parquet",
):
    """Clean the German city and state reference data."""
    data = pd.read_excel(depends_on, sheet_name="Städte", skiprows=1, skipfooter=9)
